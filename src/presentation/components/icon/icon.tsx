import React from 'react'

import Styles from './icon-styles.scss'

export enum IconNameEnum {
  thumbDown = 'data:image/png;base64,',
  thumbUp = 'data:image/png;base64,',
}

export type IconName = keyof typeof IconNameEnum

interface Props {
  iconName: IconName
  className?: string
}

export const Icon: React.FC<Props> = ({ iconName, className }) => {
  const iconColor = IconNameEnum[iconName] === IconNameEnum.thumbUp ? 'green' : 'red'

  return (
    <div className={[Styles.iconWrap, Styles[iconColor], className].join(' ')}>
      <img data-testid="icon" src={IconNameEnum[iconName]} />
    </div>
  )
}
